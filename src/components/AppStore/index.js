...
class App extends Component {
  state = {
    activeApps: tabsList[0].tabId,
    searchInput: '',
  }

  getFilterdApps = () => {
    const {activeApps} = this.state
    const filteredApps = appsList.filter(each => each.category === activeApps)
    return filteredApps
  }

  switchingTabs = tabId => {
    this.setState({activeApps: tabId, searchInput: ''})
  }
  ...
  render() {
    const {searchInput, activeApps} = this.state
    const filteredApps = this.getFilterdApps()

    const searchedResults = appsList.filter(each =>
      each.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )

    const search = searchedResults.filter(each => each.category === activeApps)
        ...
        <div className="bgColor">
          ...
          <div className="xx">
            ...
            <img
              ...
              alt=" search icon"
              className="seacrchImg"
            />
          </div>
          <ul>
            <div className="tabListInOrder">
              {tabsList.map(each => (
                <TabItem
                  key={each.tabId}
                  TabDetailes={each}
                  switchingTabs={this.switchingTabs}
                  isTabActive={each.tabId === activeApps}
                />
              ))}
            </div>
          </ul>
          <div className="appsContainer">
            {filteredApps.map(each => (
              <AppItem key={each.appId} AppList={each} />
            ))}
          </div>
          <ul>
            {search.map(each => (
